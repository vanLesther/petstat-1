...
if (isset($_POST['update'])) {
    ...
    if ($result === true) {
        // Successful updat
            echo '<script>alert("Vaccination status updated successfully"); window.location.href = "./dashboard1pet.php?active-tab=2";</script>';
        exit;
    } else {
        // Failed to update vaccination status
        echo '<script>alert("Failed to update vaccination status: ' . $result . '"); window.location.href = "./dashboard1pet.php?active-tab=2";</script>';
        exit;
    }
}
?>
